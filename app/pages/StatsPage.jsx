...
export default function StatsPage() {
  ...
  useEffect(() => {
    async function getdata() {
      ...
      setdata(result.data.api.topscorers);
    }
    getdata();
  }, []);
...
const mapPlayersToVM = memoize(function (players) {
  const mappedPlayers = [];
  ...
  players.forEach((player, index) => {
    previousPlayer = mappedPlayers[index - 1];

    currentPlayer.name = `${player.firstname} ${player.lastname}`;
    currentPlayer.team = player.team_name;
    currentPlayer.value = player.goals.total;
    currentPlayer.imageUri = getLogoUri();

    // first player: assuming the data is sorted
    if (index === 0) {
      currentPlayer.rank = 1;
    } else {
      // compare goals with previous
      currentPlayer.rank =
        currentPlayer.value < previousPlayer.value ? previousPlayer.rank + 1 : previousPlayer.rank;
    }
...
const getLogoUri = memoize(function () {
